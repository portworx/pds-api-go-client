...
> ModelsBackup ApiBackupsPost(ctx).Body(body).LocalOnly(localOnly).Execute()
...
func main() {
    body := *openapiclient.NewControllersCreateBackupRequest() // ControllersCreateBackupRequest | Request body containing the backup config
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.BackupsApi.ApiBackupsPost(context.Background()).Body(body).LocalOnly(localOnly).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `BackupsApi.ApiBackupsPost``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `ApiBackupsPost`: ModelsBackup
    fmt.Fprintf(os.Stdout, "Response from `BackupsApi.ApiBackupsPost`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiApiBackupsPostRequest struct via the builder pattern

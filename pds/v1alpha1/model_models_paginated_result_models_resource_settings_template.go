/*
PDS API

Portworx Data Services API Server

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package pds

import (
	"encoding/json"
)

// checks if the ModelsPaginatedResultModelsResourceSettingsTemplate type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ModelsPaginatedResultModelsResourceSettingsTemplate{}

// ModelsPaginatedResultModelsResourceSettingsTemplate struct for ModelsPaginatedResultModelsResourceSettingsTemplate
type ModelsPaginatedResultModelsResourceSettingsTemplate struct {
	Data []ModelsResourceSettingsTemplate `json:"data,omitempty"`
	Pagination *ConstraintPagination `json:"pagination,omitempty"`
}

// NewModelsPaginatedResultModelsResourceSettingsTemplate instantiates a new ModelsPaginatedResultModelsResourceSettingsTemplate object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewModelsPaginatedResultModelsResourceSettingsTemplate() *ModelsPaginatedResultModelsResourceSettingsTemplate {
	this := ModelsPaginatedResultModelsResourceSettingsTemplate{}
	return &this
}

// NewModelsPaginatedResultModelsResourceSettingsTemplateWithDefaults instantiates a new ModelsPaginatedResultModelsResourceSettingsTemplate object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewModelsPaginatedResultModelsResourceSettingsTemplateWithDefaults() *ModelsPaginatedResultModelsResourceSettingsTemplate {
	this := ModelsPaginatedResultModelsResourceSettingsTemplate{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *ModelsPaginatedResultModelsResourceSettingsTemplate) GetData() []ModelsResourceSettingsTemplate {
	if o == nil || IsNil(o.Data) {
		var ret []ModelsResourceSettingsTemplate
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ModelsPaginatedResultModelsResourceSettingsTemplate) GetDataOk() ([]ModelsResourceSettingsTemplate, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *ModelsPaginatedResultModelsResourceSettingsTemplate) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []ModelsResourceSettingsTemplate and assigns it to the Data field.
func (o *ModelsPaginatedResultModelsResourceSettingsTemplate) SetData(v []ModelsResourceSettingsTemplate) {
	o.Data = v
}

// GetPagination returns the Pagination field value if set, zero value otherwise.
func (o *ModelsPaginatedResultModelsResourceSettingsTemplate) GetPagination() ConstraintPagination {
	if o == nil || IsNil(o.Pagination) {
		var ret ConstraintPagination
		return ret
	}
	return *o.Pagination
}

// GetPaginationOk returns a tuple with the Pagination field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ModelsPaginatedResultModelsResourceSettingsTemplate) GetPaginationOk() (*ConstraintPagination, bool) {
	if o == nil || IsNil(o.Pagination) {
		return nil, false
	}
	return o.Pagination, true
}

// HasPagination returns a boolean if a field has been set.
func (o *ModelsPaginatedResultModelsResourceSettingsTemplate) HasPagination() bool {
	if o != nil && !IsNil(o.Pagination) {
		return true
	}

	return false
}

// SetPagination gets a reference to the given ConstraintPagination and assigns it to the Pagination field.
func (o *ModelsPaginatedResultModelsResourceSettingsTemplate) SetPagination(v ConstraintPagination) {
	o.Pagination = &v
}

func (o ModelsPaginatedResultModelsResourceSettingsTemplate) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ModelsPaginatedResultModelsResourceSettingsTemplate) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.Pagination) {
		toSerialize["pagination"] = o.Pagination
	}
	return toSerialize, nil
}

type NullableModelsPaginatedResultModelsResourceSettingsTemplate struct {
	value *ModelsPaginatedResultModelsResourceSettingsTemplate
	isSet bool
}

func (v NullableModelsPaginatedResultModelsResourceSettingsTemplate) Get() *ModelsPaginatedResultModelsResourceSettingsTemplate {
	return v.value
}

func (v *NullableModelsPaginatedResultModelsResourceSettingsTemplate) Set(val *ModelsPaginatedResultModelsResourceSettingsTemplate) {
	v.value = val
	v.isSet = true
}

func (v NullableModelsPaginatedResultModelsResourceSettingsTemplate) IsSet() bool {
	return v.isSet
}

func (v *NullableModelsPaginatedResultModelsResourceSettingsTemplate) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableModelsPaginatedResultModelsResourceSettingsTemplate(val *ModelsPaginatedResultModelsResourceSettingsTemplate) *NullableModelsPaginatedResultModelsResourceSettingsTemplate {
	return &NullableModelsPaginatedResultModelsResourceSettingsTemplate{value: val, isSet: true}
}

func (v NullableModelsPaginatedResultModelsResourceSettingsTemplate) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableModelsPaginatedResultModelsResourceSettingsTemplate) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


